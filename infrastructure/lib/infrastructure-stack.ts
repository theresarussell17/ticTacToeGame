...
export class InfrastructureStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    // Add S3 Bucket
    const s3Site = new s3.Bucket(this, `<tictactoe>`, {
      bucketName: `tictactoe-s3bucket`,
    ...
    const distribution = new cloudFront.CloudFrontWebDistribution(
        ...
        `tictactoe-cf-distribution`,
        {
          ...
          comment: `tictactoe - CloudFront Distribution`,
          viewerProtocolPolicy: cloudFront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS
        }
    );

    // Setup Bucket Deployment to automatically deploy new assets and invalidate cache
    new s3deploy.BucketDeployment(this, `tictactoe-s3bucketdeployment`, {
